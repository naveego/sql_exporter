...
func NewSQLQuery(logContext string, qc *config.QueryConfig, metricFamilies ...*MetricFamily) (Collector, errors.WithContext) {
...
func (q *SQLQuery) Collect(ctx context.Context, db DB, ch chan<- Metric) {
	if ctx.Err() != nil {
		ch <- NewInvalidMetric(errors.Wrap(q.logContext, ctx.Err()))
		return
	}

	conn, dberr := db.SQL(q.logContext)
	if dberr != nil {
		ch <- NewInvalidMetric(errors.Wrap(q.logContext, dberr))
		return
	}
...
func (q *SQLQuery) run(ctx context.Context, conn *sql.DB) (*sql.Rows, errors.WithContext) {
...
func (q *SQLQuery) scanDest(rows *sql.Rows) ([]interface{}, errors.WithContext) {
...
func (q *SQLQuery) scanRow(rows *sql.Rows, dest []interface{}) (map[string]interface{}, errors.WithContext) {
	columns, err := rows.Columns()
	if err != nil {
		return nil, errors.Wrap(q.logContext, err)
	}
